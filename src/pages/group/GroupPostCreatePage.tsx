...
export default function GroupPostCreatePage() {
  ...
  const removeTag = (index: number) =>
    setHashTag((prev) => [...prev.slice(0, index), ...prev.slice(index + 1)]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (imgFiles.length > 0) {
        ...
        const { error: filesErr } = await supabase.from("files").insert(rows);
        ...
        <div className="flex flex-col gap-5.5 px-6 py-4 rounded-xl bg-white shadow-[0_4px_20px_rgba(0,0,0,0.05)]">
          ...
          <div className="relative px-6 py-4 rounded-xl bg-white border-1 border-[#E5E7EB]">
            <select
              ...
              required
              value={boardType}
              onChange={(e) => setBoardType(e.target.value as BoardKey)}
            >
          ...
          <InputFile imgFiles={imgFiles} setImgFiles={setImgFiles} isMulti={true} />
