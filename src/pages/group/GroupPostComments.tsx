...
export default function GroupPostComments({
  ...
  function Comment({ comment }: { comment: CommentRowForGroup }) {
    ...
    return (
      <div
        className={
          "relative group w-full px-4 py-3 mt-4 rounded-xl bg-white shadow-[0_4px_20px_rgba(0,0,0,0.05)] " +
          adoptedStyle
        }
      >
        <button
          ...
          className={`absolute -top-4 right-4 z-10 px-3 py-1 text-xs rounded-2xl border-1 border-[#8B5CF6] bg-white
                      transition-all duration-200
                      ${canAdopt ? "opacity-0 -translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto" : "hidden"}`}
        >
          채택
        </button>
        ...
        <p className="mt-3 mb-2 text-sm text-[#6B7280]">
  ...
  return (
    <>
      ...
      <form className="flex gap-2 mt-4 w-full" onSubmit={writeComment}>
        <div className="w-[696px] text-sm px-6 py-3 border-1 border-[#E5E7EB] rounded-xl bg-white" onKeyDown={handleKeyDown}>
          {mention.nickname && (
