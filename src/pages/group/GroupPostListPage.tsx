...
export default function GroupPostListPage() {
  ...
  const { currentUserId, isLoggedIn } = useProfileStore();
  ...
  const totalPages = useMemo(
    () => Math.max(1, Math.ceil(safePosts.length / postsPerPage)),
    [safePosts.length],
  );
  ...
  const displayedPosts = useMemo(
    ...
    [safePosts, currentPage, postsPerPage],
  );
  ...
  const fetchGroupPosts = async (gid: string, tab: string) => {
    ...
    if (tab === "activity") query = query.eq("group_board_type", "activity");
    ...
    (async () => {
      try {
        ...
        if (activeTab === "members") {
          const memberList = await fetchUserFollowings(currentUserId, g.id);
          if (!alive) return;
  ...
  }, [groupParam, activeTab, isLoggedIn, currentUserId, navigate, fetchUserFollowings]);


  if (loading || !isLoggedIn || !currentUserId) {
    return (
      ...
      <div className="flex items-center justify-between">
        ...
        {activeTab !== "attendance" &&
          ...
          group?.id && (
            <button
              type="button"
              onClick={() => navigate(`/groups/${group.id}/posts/create`)}
        ...
        {(activeTab === "notice" || activeTab === "activity") && (
          <div>
            {safePosts.length === 0 ? (
              ...
              <PostList posts={displayedPosts as any} groupId={group?.id} />
            )}
