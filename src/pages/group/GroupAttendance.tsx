...
export default function GroupAttendancePage() {
  ...
  const ensureAttendancePost = async (gid: string, dateISO: string, authorId: string) => {
    ...
    if (found && found.length > 0 && found[0]?.id) return found[0].id as string;
  ...
  const fetchCommentsWithAuthor = async (pid: string) => {
    ...
    const base: CommentVM[] = (data ?? []).map((c: any) => ({
      ...
      user: {
        ...
        major:
          c.users?.major ??
          (c.user_id === currentUserId ? (myProfile as any)?.major ?? null : null),
        badge: null,
      },
    }));
    ...
    const badgeLogIds = Array.from(
      new Set(
        base.map((b) => b.user.representative_badge_id).filter(Boolean) as string[],
      ),
    );
    ...
    if (badgeLogIds.length > 0) {
      ...
      for (const row of logs ?? []) {
        if (row?.id && row?.badge_id) badgeIdByLogId.set(row.id, row.badge_id);
      }
    }
    ...
    for (const item of base) {
      ...
      if (logId && badgeIdByLogId.has(logId)) badgeIds.add(badgeIdByLogId.get(logId)!);
    }
    ...
    if (badgeIds.size > 0) {
      ...
      for (const b of badges ?? []) {
        badgeById.set(b.id, { id: b.id, name: b.name, icon_url: b.icon_url ?? null });
      }
    }
    ...
    for (const item of base) {
      ...
      if (logId && badgeIdByLogId.has(logId)) badgeId = badgeIdByLogId.get(logId)!;
      if (!badgeId && latestBadgeIdByUser.has(item.user_id)) badgeId = latestBadgeIdByUser.get(item.user_id)!;
    ...
    try {
      ...
      const optimistic: CommentVM = {
        ...
        user: {
          ...
          profile_image_url: (myProfile as any)?.profile_image_url ?? null,
          representative_badge_id: (myProfile as any)?.representative_badge_id ?? null,
      ...
      const { error: insErr } = await supabase.from("comments").insert([
        { post_id: postId, user_id: currentUserId, content },
      ]);
      if (insErr) throw insErr;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const pid = await ensureAttendancePost(g.id, todayISO, currentUserId);
        if (!alive) return;
        setPostId(pid);
  ...
  return (
    <div className="w-[920px] mx-auto">
      <div className="mt-6 mb-4 border-b border-gray-200 pb-3">
        <h3 className="text-[20px] font-semibold">{todayKorean} 출석 현황</h3>
      </div>

      {comments.length === 0 ? (
        <p className="text-center text-gray-500 py-16">아직 아무도 출석하지 않았습니다…</p>
      ) : (
        <ul className="space-y-3">
          {comments.map((c) => (
            <li key={c.id} className="rounded-xl bg-white border border-gray-200 px-4 py-3 shadow-sm">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-full bg-gray-200 overflow-hidden">
                    {c.user.profile_image_url ? (
                      <img src={c.user.profile_image_url} alt={c.user.nickname} className="h-8 w-8 object-cover" />
                    ) : null}
                  </div>
                  <div>
                      ...
                      {c.user.badge ? (
                        <span className="inline-flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded bg-yellow-100 text-yellow-700">
                          {c.user.badge.icon_url ? (
                            <img src={c.user.badge.icon_url} alt={c.user.badge.name} className="h-3 w-3 object-contain" />
                          ) : null}
                          {c.user.badge.name}
                        </span>
                      ) : null}
                      {c.user.major ? <span className="text-[12px] text-gray-500">{c.user.major}</span> : null}
                    </div>
                    <p className="text-[13px] text-gray-600 mt-1 break-words">{c.content}</p>
                  </div>
                </div>
      ...
      <div className="flex items-center gap-2 mt-6">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" && !e.nativeEvent.isComposing) submitAttendanceComment();
          }}
        />
        <button
          type="button"
