...
async function safeGetUser() {
  ...
  if (error && (error.message?.includes("Auth session missing") || (error as any).status === 400)) {
    return null;
  }
...
async function reqJoin(groupId: string) {
  ...
  if (error && (error as any).code !== "23505") throw error; // 중복 가입은 무시
}
...
export default function GroupPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (msg.includes("Auth session missing") || (e as any)?.status === 400) {
  ...
  const toggleJoin = async (g: GroupView, wantJoin: boolean) => {
    setAll((prev) =>
      prev.map((x) =>
        x.id === g.id
          ? { ...x, member_count: x.member_count + (wantJoin ? 1 : -1), joined: wantJoin }
          : x
      )
    );
    setMine((prev) =>
      wantJoin
        ? prev.some((x) => x.id === g.id)
          ? prev
          : [...prev, { ...g, joined: true, member_count: g.member_count + 1 }]
    ...
    <div className="max-w-[1280px] mx-auto px-6 md:px-8">
      ...
      <section className="mt-8 md:mt-10">
        <div className="flex items-center justify-between">
          <h2 className="text-xl md:text-2xl font-semibold tracking-tight">내가 활동 중인 그룹</h2>
              ...
              return (
                <div key={g.id} className="space-y-2">
                  ...
                  <div className="flex justify-end">
                    <button
                      onClick={() => toggleJoin(g, false)}
                      className="h-7 rounded-lg px-3 text-xs font-medium text-white bg-gray-500 hover:bg-gray-600"
                    >
                      탈퇴하기
                    </button>
                  </div>
            ...
            {allGroups.map((g) => {
              ...
              return (
                <div key={g.id} className="space-y-2">
                  ...
                  <div className="flex justify-end">
                    <button
                      onClick={() => toggleJoin(g, !g.joined)}
                      className={`h-7 rounded-lg px-3 text-xs font-medium text-white ${
                        g.joined ? "bg-gray-500 hover:bg-gray-600" : "bg-[#8B5CF6] hover:bg-[#7C3AED]"
                      }`}
                    >
                      {g.joined ? "탈퇴하기" : "참여하기"}
                    </button>
